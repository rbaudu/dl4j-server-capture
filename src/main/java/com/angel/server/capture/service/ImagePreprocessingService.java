...
public class ImagePreprocessingService {
     ...
     * Génère 64 canaux comme attendu par le modèle
     */
    public INDArray preprocessForPresenceDetection(BufferedImage image) {
        try {
            ...
            long[] expectedShape = {1, targetChannels, targetHeight, targetWidth};
     ...
     * Utilise uniquement les API Java standard
     */
    ...
    /**
     * Alternative avec normalisation centrée et réduite pour 64 canaux
     */
    public INDArray preprocessForPresenceDetectionNormalized(BufferedImage image) {
        try {
            ...
            // Normalisation centrée et réduite: (x - 0.5) / 0.5 pour chaque canal
            return standard.mul(2.0f).sub(1.0f);
    ...
    /**
     * Preprocessing avec normalisation par canal (standardisation)
     */
    public INDArray preprocessForPresenceDetectionStandardized(BufferedImage image) {
        try {
            INDArray features = preprocessForPresenceDetection(image);
            if (features == null) return null;
            
            // Standardiser chaque canal séparément (mean=0, std=1)
            int channels = (int) features.shape()[1];
            for (int c = 0; c < channels; c++) {
                INDArray channel = features.slice(0, 0).slice(c, 1); // [height, width]
                double mean = channel.meanNumber().doubleValue();
                double std = channel.stdNumber().doubleValue();
                
                if (std > 0) {
                    channel.subi(mean).divi(std);
                }
            }
            ...
            logger.error("Erreur lors du preprocessing standardisé: {}", e.getMessage(), e);
            return null;
        }
    }
    ...
    public INDArray preprocessWithNormalization(BufferedImage image, String normalizationType) {
        switch (normalizationType.toLowerCase()) {
            ...
            case "standardized":
                return preprocessForPresenceDetectionStandardized(image);
            case "standard":
            default:
    ...
    public void debugPreprocessing(BufferedImage image) {
        ...
        logger.info("Configuration cible: {} canaux, {}x{}", targetChannels, targetWidth, targetHeight);
        ...
        INDArray standardized = preprocessForPresenceDetectionStandardized(image);
        if (standardized != null) {
            logger.info("Standardized - Shape: {}, Min: {:.3f}, Max: {:.3f}, Mean: {:.3f}", 
                       Arrays.toString(standardized.shape()),
                       standardized.minNumber().floatValue(),
                       standardized.maxNumber().floatValue(),
                       standardized.meanNumber().floatValue());
        }
